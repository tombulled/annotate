...
def test_annotate_single(func: types.FunctionType, annotation: annotate.Annotation) -> None:
    annotate.annotate(func, annotation)

    assert annotate.get_raw_annotations(func) == {annotation.key: annotation}

def test_annotate_multiple(func: types.FunctionType, annotations: typing.List[annotate.Annotation]) -> None:
...
def test_annotate_inherited(cls: type) -> None:
    annotation_a: annotate.Annotation = annotate.Annotation('key-a', 'value-a', inherited=False)
    annotation_b: annotate.Annotation = annotate.Annotation('key-b', 'value-b', inherited=True)
...
def test_annotate_targetted(func: types.FunctionType) -> None:
    annotation_func: annotate.Annotation = annotate.Annotation('key-a', 'value-a', targets=(types.FunctionType,))
    annotation_type: annotate.Annotation = annotate.Annotation('key-b', 'value-b', targets=(type,))
